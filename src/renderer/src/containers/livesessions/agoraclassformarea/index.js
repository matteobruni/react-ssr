...
function AgoraClassFormArea() {
  ...
  const handleSubmit = async () => {
    ...
    let subjectGenericName = selectedSubject?.generic_name;

    if(selectedCategory.skippable) subjectGenericName = selectedCategory.generic_name;

    const gradient = getColorGradientForSubject(subjectGenericName);
    ...
    const obj = {
      ...
      class_type: 'live_interactive_class',
      category: {
    ...
    try {
      ...
      // return;
      // const sessionCRUD = Session(obj, fileData?.value, curSession.sessionObj, {...curSession, ...curSessionDetails}, ({progress, bytesTransferred, totalBytes}) => {
      //   setProgress(progress);
      // }, () => {
      //   if(doEdit && !doClone) {
      //     setStatus('editting');
  ...
  const doLive = useMemo(() => {
    return query.get('live') === 'true';
  }, [query]);
  ...
  const gradeInUrl = useMemo(() => {
    ...
    return grade === 'undefined' || grade === 'null' ? false : grade;
  }, [query]);
  ...
  }, [sessionIdInUrl, gradeInUrl, handleChange]);
  ...
  }, [handleChange, fetchedSessionObj]);
  ...
  useEffect(() => {
    ...
    console.log('doEdit - ', doEdit, gradeItems, curSession, curSessionDetails);
...
export default AgoraClassFormArea;
